...
{
    const string apiKey = "c62c571c889d1edc580f6667a877e1af";

    public static string GetFormated(int value, double amount = 1000.0) => value == 0 ? "-" : $"{value / amount:N1}k";

    public static string GetFormated(long value, double amount = 1000.0) => value == 0 ? "-" : $"{value / amount:N1}k";

    public static async Task<string> LastGame(RecentMatches match)
    {
...
<html>
    ...
    public static async Task<string> LastGameFull(Match match)
    {
...
";
    ...
    private static string GetMatchTable(IEnumerable<MatchPlayer> players)
    {
        ...
        foreach (var player in players)
        {
            ...
            sb.AppendLine($"<td><img src='{DotaApi.GetHeroThumbnail(player.HeroId.Value)}' Height='100' Width='178'/></td>");
            ...
            sb.AppendLine($"<td>{GetFormated(player.HeroDamage.Value)}/{GetFormated(player.TowerDamage.Value)}</td>");
            sb.AppendLine($"<td>{GetFormated(player.HeroHealing.Value)}</td>");
            sb.AppendLine("</tr>");
        }
        return sb.ToString();
    }

    public static async Task<string> GetImageUrl(string html)
    {
        var converter = new HtmlConverter();
        var bytes = converter.FromHtmlString(html);
        var image = Convert.ToBase64String(bytes);
        string endPoint = @$"https://api.imgbb.com/1/upload?expiration=600&key={apiKey}";
